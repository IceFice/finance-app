...
export async function list(userId: string) {
  const res = await pool.query<CategoryRow>(
    `SELECT * FROM categories WHERE (user_id IS NULL OR user_id = $1) ORDER BY is_system DESC, sort_order ASC, name ASC`,
    [userId]
  );
...
export async function update(userId: string, categoryId: string, input: UpdateCategoryInput) {
  const check = await pool.query<CategoryRow>(`SELECT * FROM categories WHERE id = $1`, [categoryId]);
  if (!check.rows[0]) throw new NotFoundError('Category');
  if (check.rows[0].is_system || check.rows[0].user_id !== userId) throw new ForbiddenError('Cannot edit system category');
...
export async function remove(userId: string, categoryId: string) {
  const check = await pool.query<CategoryRow>(`SELECT * FROM categories WHERE id = $1`, [categoryId]);
  if (!check.rows[0]) throw new NotFoundError('Category');
  if (check.rows[0].is_system || check.rows[0].user_id !== userId) throw new ForbiddenError('Cannot delete system category');
